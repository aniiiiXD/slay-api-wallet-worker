/**
 * CBTC Token Standard registry client.
 *
 * Endpoint pattern (per BitSafe docs):
 *
 *   GET {CBTC_REGISTRY_URL}/api/token-standard/v0/registrars/{admin}/registry/metadata/v1/instruments
 *
 * Returns metadata for every instrument the admin party governs. For
 * BitSafe's `cbtc-network` party on MainNet that's just CBTC today;
 * future siblings (e.g. CBTC.L2 for a future L2 variant) would appear
 * in the same response.
 *
 * No auth needed — registry data is public. We just need the registry
 * URL + admin party id from env.
 *
 * Drift safety — BitSafe explicitly warns instrument metadata can change
 * (DAR upgrades, MainNet topology migrations, infrastructure rotations)
 * and there's no push notification. Three guarantees we make here:
 *
 ...
 *      `[cbtc.registry] DRIFT` lines in their tail when a DAR upgrade
 *      lands.
 *   3. If a refresh fetch fails (network blip, BitSafe 5xx), serve the
 ...
 *      take down our CBTC paths if we have a recent value to keep
 *      going on.
 *
 * Process-local cache with a 10 min TTL — fits inside BitSafe's 5–15
 * min recommended poll window. On Workers each cold start re-fetches
 * naturally, which gives us the docs-recommended "always fetch fresh
 * on startup" behaviour for free.
 */
...
type RegistryInstrumentsResponse = {
  instruments?: Array<{
    // BitSafe MainNet (June 2026) returns the instrument id as a
    // top-level `id` field, NOT nested under `instrument_id`. Their
    // public docs at https://docs.bitsafe.finance/... show a nested
    // shape; reality differs. We accept BOTH so the code survives
    // whichever shape the registry decides to ship.
    id?: string;
...
export class CbtcConfigError extends Error {}
...
function requireEnv(env: Env): {
  ...
  if (!env.CBTC_REGISTRY_URL) {
    throw new CbtcConfigError(
      "CBTC_REGISTRY_URL not set. Add the BitSafe registry URL to env (MainNet: https://api.utilities.digitalasset.com)."
    );
  }
  if (!env.CBTC_ADMIN_PARTY) {
    throw new CbtcConfigError(
      "CBTC_ADMIN_PARTY not set. Add BitSafe's CBTC admin party id (MainNet: cbtc-network::12205af3b949…)."
    );
  }
  if (!env.CBTC_INSTRUMENT_ID) {
    throw new CbtcConfigError(
      'CBTC_INSTRUMENT_ID not set. For CBTC use the literal string "CBTC".'
    );
  }
  return {
    registryUrl: env.CBTC_REGISTRY_URL.replace(/\/$/, ""),
    adminParty: env.CBTC_ADMIN_PARTY,
    instrumentId: env.CBTC_INSTRUMENT_ID,
  };
}

/**
 * Fetch and cache CBTC instrument metadata from BitSafe's registry.
 *
 ...
 * BitSafe rotates we see it in tail rather than discovering via
 * transfer failures.
 */
export async function getCbtcInstrument(env: Env): Promise<InstrumentMetadata> {
  const now = Date.now();
  ...
  if (!res.ok) {
    return serveStaleOrThrow(
      `BitSafe registry returned ${res.status} for ${url}`
    );
  }
  ...
  try {
    ...
    return serveStaleOrThrow(
      `BitSafe registry returned malformed JSON: ${err instanceof Error ? err.message : String(err)}`
    );
  }
  ...
  const meta: InstrumentMetadata = {
    ...
    name: hit.name,
    symbol: hit.symbol,
    // BitSafe is currently shipping CBTC with decimals=10 (not the BTC-
    // standard 8). Use whatever the registry says; if absent default to
    // 8 (matches the older docs assumption).
  ...
  if (cache) {
    ...
    if (before !== after) {
      console.warn(
        `[cbtc.registry] DRIFT — instrument signature changed.\n  before: ${before}\n  after:  ${after}\n  (Most likely a BitSafe DAR upgrade. Verify env CBTC_ADMIN_PARTY is still correct.)`
      );
    }
    if (meta.instrumentId.admin !== adminParty) {
      console.warn(
        `[cbtc.registry] ADMIN MISMATCH — env CBTC_ADMIN_PARTY=${adminParty.slice(0, 32)}… but registry returned ${meta.instrumentId.admin.slice(0, 32)}…. Update the env var when convenient; transfers will use the resolved value.`
      );
    }
  }
  ...
  if (env.SPLICE_DEBUG === "1") {
    console.log(
      `[cbtc.registry] cached metadata — symbol=${meta.symbol ?? "?"} decimals=${meta.decimals} package=${meta.packageId?.slice(0, 24)}…`
    );
  }
  return meta;
}
 ...
 *  BitSafe Token Standard exposes an active-holdings query per owner    *
 *  party. We use this instead of our participant's /v2/state/active-    *
 *  contracts because:                                                    *
 *                                                                        *
 *   - Canton 3.5.1's JSON v2 API hardcodes a 200-element response cap   *
 *     (JSON_API_MAXIMUM_LIST_ELEMENTS_NUMBER_REACHED). The operator     *
 *     party crosses that ceiling easily because it owns ValidatorRight  *
 *     contracts for every managed user (100+ bots + N customers) plus   *
 *     TransferPreapprovals, WalletAppInstalls, and CC change Amulets.   *
 *   - The participant silently ignores TemplateFilter / InterfaceFilter *
 *     in our setup, so we can't narrow the result server-side.          *
 *   - BitSafe's registry has its OWN indexer that knows which CBTC      *
 *     Holdings exist on which parties. It returns ONLY CBTC contracts   *
 *     by definition — no ValidatorRights, no Amulets, no noise. No cap. *
 *                                                                        *
 *  This is what makes operator-side CBTC transfers (treasury → user)    *
 *  possible at all once the operator's contract count crosses 200.      *
 * ──────────────────────────────────────────────────────────────────── */
...
type RegistryHoldingsResponse = {
  /** BitSafe's docs show `holdings[]` as the standard envelope.
   *  Accept a few field-name variants for resilience. */
...
/**
 * Fetch active CBTC holdings owned by a specific party, via BitSafe's
 * registry holdings endpoint. Bypasses the participant's 200-element
 * cap entirely — the registry returns ALL of the party's CBTC holdings
 * regardless of count, since it's a token-scoped index.
 *
 * Optional `instrumentIdOverride` lets callers force a specific
 * instrument; default uses the env's CBTC_INSTRUMENT_ID.
 *
...
export async function getActiveHoldings(
  ...
  // BitSafe's Token Standard holdings query body. Field names match
  // the OpenAPI spec at docs.digitalasset.com — owner_party_id /
  // instrument_id (snake_case, matching the rest of their API).
  const body = {
  ...
  try {
    ...
    throw new Error(
      `BitSafe registry holdings query network error (${url}): ${err instanceof Error ? err.message : String(err)}`
    );
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `BitSafe registry holdings query ${res.status} from ${url}: ${errBody.slice(0, 500)}`
    );
  }
  ...
  // exact field names from BitSafe vary between implementations
  // (camelCase vs snake_case, nested vs top-level). We try a few.
...
function serveStaleOrThrow(reason: string): InstrumentMetadata {
  ...
  if (cache && now - cache.lastFreshAt < STALE_FALLBACK_MAX_MS) {
    console.warn(
      `[cbtc.registry] STALE — serving cached metadata (lastFreshAt=${new Date(cache.lastFreshAt).toISOString()}). reason=${reason}`
    );
  ...
  throw new CbtcConfigError(reason);
}
