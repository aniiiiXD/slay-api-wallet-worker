/**
 * HECTO holding cid cache — see comment block on schema.hectoHoldingCache
 ...
 * participant for "which HECTO Holding does treasury currently own", so
...
export async function getCachedHolding(
  ...
  const [row] = await db
    .select({
      partyId: schema.hectoHoldingCache.partyId,
      holdingCid: schema.hectoHoldingCache.holdingCid,
      amountRaw: schema.hectoHoldingCache.amountRaw,
      updatedAt: schema.hectoHoldingCache.updatedAt,
    })
    .from(schema.hectoHoldingCache)
    .where(eq(schema.hectoHoldingCache.partyId, partyId))
    .limit(1);
  if (!row) return null;
  return row;
}
 ...
 *  1. POST /api/admin/hecto/seed-holding-cid — initial operator setup
 *  2. transferHecto after a successful transfer — picks up the new
...
export async function setCachedHolding(
  ...
  await db
    .insert(schema.hectoHoldingCache)
    .values({
      partyId,
      holdingCid,
      amountRaw,
    ...
    .onConflictDoUpdate({
      target: schema.hectoHoldingCache.partyId,
      set: {
        holdingCid,
        amountRaw,
        updatedAt: sql`NOW()`,
...
export async function getCachedHoldingAmounts(
  ...
  const rows = await db
    ...
    .from(schema.hectoHoldingCache)
    .where(inArray(schema.hectoHoldingCache.partyId, partyIds));
...
export async function invalidateCachedHolding(
  ...
  await db
    .delete(schema.hectoHoldingCache)
    .where(eq(schema.hectoHoldingCache.partyId, partyId));
}
