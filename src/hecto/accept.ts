/**
 * HECTO TransferInstruction Accept — receiver-side completion of a
 * two-phase HECTO transfer.
 *
 * Why this exists: TransferFactory_Transfer creates a
 * `TransferInstruction` contract on chain rather than a direct Holding.
 * The HECTO doesn't actually land in the receiver's wallet until the
...
export async function getAcceptContext(
  ...
  if (!res.ok) {
    ...
    throw new LedgerCallError(
      res.status,
      `HECTO accept-context registry rejected: ${res.status} ${text.slice(0, 240)}`
    );
  }
...
async function submitAcceptCommand(
  ...
  const commandId = `hecto-accept-${transferInstructionCid.slice(0, 16)}-${Date.now()}`;
  ...
  if (env.TRAFFIC_DEBUG === "1") {
    ...
    console.log(
      `[traffic] choice=HECTO:TransferInstruction_Accept bodyBytes=${bodyBytes} disclosedBytes=${discBytes} disclosedCount=${disc.length}`
    );
  }
  ...
  //
  // Same retry shape as src/hecto/transfer.ts. Bounded: 6 attempts with
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    console.log(
      `[hecto.accept] sequencer 409 attempt ${attempt}/${MAX_ATTEMPTS}, retry in ${sleepMs}ms — cid=${commandId.slice(0, 16)}…`
    );
...
export async function acceptTransferInstruction(
  ...
  if (env.HECTO_ENABLED !== "1" && env.HECTO_ENABLED !== "true") {
    throw new LedgerCallError(503, "HECTO is not enabled.");
  }
  ...
  // HECTO ships with decimals=10. The registry's `decimals` field is the
  ...
  for (const ev of result.createdEvents) {
    ...
    if (arg.amount != null) {
      const hecto = Number(arg.amount);
      if (Number.isFinite(hecto)) {
        amountSat = Math.round(hecto * conversionFactor);
      }
    }
    break;
  }
