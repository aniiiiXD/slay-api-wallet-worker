/**
 * HECTO transfer — Splice Token Standard `TransferFactory_Transfer` exercise.
 *
 * Mirrors hecto's CBTC sibling. The pattern:
 *
 ...
 *
 * AUTH: Bearer token = onrails HECTO Keycloak access token (see
...
import type { HectoTransferResult, InstrumentId } from "./types";
import { getHectoInstrument } from "./registry";
import { discoverOperatorHoldingsViaLedger } from "../canton/operator-holdings";
import { getHectoKeycloakToken, invalidateKeycloakToken } from "./keycloak";
 ...
 *  consume. We list the sender's HECTO Holdings, then pass ALL of them   *
 ...
 *  The REAL workaround for the operator is the hecto_holding_cache table *
 *  (see holding-cache.ts). Treasury hits the cache; user parties (which *
 *  have <200 active contracts each) work fine via wildcard.             *
 * ──────────────────────────────────────────────────────────────────── */

/**
 * Public, cache-bypassing variant of listSenderHectoHoldings. Used by the
 * merge-fat-holdings admin endpoint to enumerate ALL active HECTO Holdings
...
export async function listAllHoldingsForPartyFresh(
  ...
  return listSenderHectoHoldingsImpl(env, party, /* skipCache */ true);
}

export async function listSenderHectoHoldings(
  env: Env,
  senderParty: string,
  ...
  void resolvedAdmin;
  return listSenderHectoHoldingsImpl(env, senderParty, /* skipCache */ false, minAmountSat);
}

async function listSenderHectoHoldingsImpl(
  env: Env,
  senderParty: string,
  skipCache: boolean,
  minAmountSat?: number
  ...
  // Plan B for the 200-element cap: if the hecto_holding_cache table
  ...
  // transferHecto with the new "change Holding" cid, so it stays current
  ...
  if (!skipCache) {
    try {
      ...
      if (cached && (minAmountSat === undefined || cached.amountRaw >= minAmountSat)) {
        console.log(
          `[hecto.transfer] listSenderHectoHoldings cache HIT sender=${senderParty.slice(0, 30)} cid=${cached.holdingCid.slice(0, 24)}… amount=${cached.amountRaw}`
        );
        return [cached.holdingCid];
      }
      console.log(
        `[hecto.transfer] listSenderHectoHoldings cache ${cached ? "INSUFFICIENT" : "MISS"} sender=${senderParty.slice(0, 30)} need=${minAmountSat ?? "?"} — walking ledger`
      );
    } catch (err) {
      // DB failures shouldn't break transfers — fall through to participant.
      console.warn(
        `[hecto.transfer] holding-cache read failed sender=${senderParty.slice(0, 30)}: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  }
  // ── End cache fast-path ──
  ...
  if (operatorParty && senderParty === operatorParty) {
    ...
    if (cids.length === 0) {
      throw new LedgerCallError(
        500,
        `Operator ${senderParty.slice(0, 32)}… has no spendable HECTO Holdings (ledger walk).`
      );
    }
    return cids;
  }
  ...
  if (!ledgerToken) {
    throw new LedgerCallError(
      500,
      "JSON_LEDGER_ADMIN_TOKEN not set — required to enumerate the sender's HECTO Holdings."
    );
  }

  // Resolve the HECTO Holding template + interface ids. Both are
  ...
  // hecto_holding_cache table (see holding-cache.ts). Treasury hits the
  // cache; user parties (which have <200 active contracts each) work
  // fine via wildcard, no filter needed.
  const instrument = await getHectoInstrument(env);
  const packageId =
    instrument.packageId ?? env.HECTO_HOLDING_TEMPLATE_PACKAGE_ID ?? null;
  const hectoHoldingTemplateId = packageId
    ? `${packageId}:Utility.Registry.Holding:Holding`
    : null;
  const hectoHoldingInterfaceId = env.HECTO_HOLDING_INTERFACE_PACKAGE_ID
    ? `${env.HECTO_HOLDING_INTERFACE_PACKAGE_ID}:Splice.Api.Token.HoldingV1:Holding`
    : null;
  ...
  // is skipped — they hit the hecto_holding_cache fast-path above.
  // Query with the Holding INTERFACE view populated so we can read each
  // holding's instrumentId reliably. Canton ignores the filter's *narrowing*
  // (it returns every contract the party sees), so we MUST filter client-side
  // by instrument — otherwise a user who also holds CBTC/cETH would have those
  // holdings passed to the HECTO factory, which rejects the mixed set with
  ...
  void hectoHoldingTemplateId;
  void hectoHoldingInterfaceId;
  ...
  const templateLabel = hectoHoldingTemplateId
    ? hectoHoldingTemplateId.slice(0, 40) + "…"
    : "<wildcard>";
  if (!res.ok) {
    const errBody = await res.text().catch(() => "<read failed>");
    console.error(
      `[hecto.transfer] listSenderHectoHoldings ${res.status} from ${ledgerHost} ` +
  ...
  console.log(
    `[hecto.transfer] listSenderHectoHoldings ok sender=${senderParty.slice(0, 30)} ` +
  ...
  for (const entry of entries) {
    ...
    // Read the Holding-interface view (populated because we request
    // includeInterfaceView). Fall back to the raw create argument. Only
    // token-standard Holdings implement the interface, so entries without a
    // view (Amulet, ValidatorRight, …) are naturally excluded.
    ...
    // CRITICAL: only THIS instrument's holdings. The participant ignores the
    // server-side filter's narrowing, so a party that also holds CBTC/cETH
    // would otherwise leak those cids into the HECTO factory call, which
    // rejects the mixed set with "Given holdings are invalid".
...
type TransferPayload = {
   ...
   *  registry (HECTO ships at decimals=10). */
  amount: string;
  ...
  /** Sender's HECTO holding cids the factory may consume.
...
async function getTransferFactory(
  ...
  const registryUrl = env.HECTO_REGISTRY_URL!.replace(/\/$/, "");
  // Use the RESOLVED admin from getHectoInstrument(), not the raw env
  ...
  if (!res.ok) {
    ...
    throw new LedgerCallError(
      res.status,
      `HECTO transfer-factory registry rejected: ${res.status} ${text.slice(0, 240)}`
    );
  }
...
async function submitTransferCommand(
  ...
  if (!ledgerHost) {
    throw new LedgerCallError(
      500,
      "JSON_API_URL not set. The HECTO transfer needs the same ledger host as CC."
    );
  }
  // Same reason as hecto/accept.ts: the participant rejects the Keycloak
  ...
  // every Token Standard asset — same for CBTC and HECTO.
    ...
    commands: [
      {
        ExerciseCommand: {
          ...
          choiceArgument: {
            ...
            // handling) — env.HECTO_ADMIN_PARTY may be stale if the
  ...
  if (env.TRAFFIC_DEBUG === "1") {
    ...
    console.log(
      `[traffic] choice=HECTO:TransferFactory_Transfer bodyBytes=${bodyBytes} disclosedBytes=${discBytes} disclosedCount=${disclosedContracts.length}`
    );
  }
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    console.log(
      `[hecto.transfer] sequencer 409 attempt ${attempt}/${MAX_ATTEMPTS}, retry in ${sleepMs}ms — cid=${commandId.slice(0, 12)}…`
    );
...
export async function transferHecto(
  env: Env,
  senderParty: string,
  ...
  // ── Validation
  if (env.HECTO_ENABLED !== "1" && env.HECTO_ENABLED !== "true") {
    throw new LedgerCallError(
      503,
      "HECTO is not enabled in this environment. Set HECTO_ENABLED=1 once credentials + participant config are in place."
    );
  }
  if (amountSat <= 0) {
    throw new LedgerCallError(400, "HECTO transfer amount must be positive.");
  }
  ...
  if (!env.HECTO_ADMIN_PARTY) {
    throw new LedgerCallError(500, "HECTO_ADMIN_PARTY not set.");
  }
  ...
  // env.HECTO_ADMIN_PARTY is just the bootstrap hint to find the right
  ...
  // claim. The decimals field is ALSO live-resolved — HECTO ships at
  ...
  const accessToken = await getHectoKeycloakToken(env);

  // ── Pre-pick sender's HECTO holdings. The registry rejects empty
  //    arrays with "No holdings provided" — they want us to list the
  //    candidate UTXOs the factory may consume.
  const inputHoldingCids = await listSenderHectoHoldings(
    env,
    senderParty,
    resolvedAdmin,
    amountSat
  ...
  if (inputHoldingCids.length === 0) {
    throw new LedgerCallError(
      400,
      `Sender ${senderParty.slice(0, 32)}… has no spendable HECTO Holdings.`
    );
  }
  ...
  const transfer: TransferPayload = {
    ...
    // pass ALL HECTO Holdings owned by the sender; the factory picks
  ...
  try {
    ...
    // re-seed from a fresh inbound HECTO.
    ...
    if (
      ...
      try {
        ...
        console.warn(
          `[hecto.transfer] cache INVALIDATE (registry stale-cid) sender=${senderParty.slice(0, 30)} ` +
            `— registry rejected pre-flight: "${msg.slice(0, 120)}". Cache cleared.`
        );
      } catch (cacheErr) {
        console.error(
          `[hecto.transfer] failed to invalidate stale cache sender=${senderParty.slice(0, 30)}: ${cacheErr instanceof Error ? cacheErr.message : String(cacheErr)}`
        );
      }
    }
    throw err;
  ...
  try {
    ...
    // accept-cron (when an inbound HECTO lands) OR fails fast with
    ...
    if (isStaleCid) {
      try {
        ...
        await invalidateCachedHolding(db, senderParty);
        console.warn(
          `[hecto.transfer] cache INVALIDATE (stale cid recovery) sender=${senderParty.slice(0, 30)} ` +
  ...
  //   (b) Two-phase TransferInstruction — receiver's HECTO sits in a
  ...
  try {
    ...
    if (senderChangeCid) {
      ...
      console.log(
        `[hecto.transfer] cache UPDATE sender=${senderParty.slice(0, 30)} ` +
      ...
      await invalidateCachedHolding(db, senderParty);
      console.warn(
        `[hecto.transfer] cache INVALIDATE sender=${senderParty.slice(0, 30)} ` +
          `— no senderChangeCid in transaction tree; next call must re-seed`
      );
    }
  } catch (err) {
    console.error(
      `[hecto.transfer] cache update failed sender=${senderParty.slice(0, 30)}: ${err instanceof Error ? err.message : String(err)}`
    );
  }
