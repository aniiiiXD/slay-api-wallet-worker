/**
 * cETH TransferInstruction Accept — receiver-side completion of a
 * two-phase cETH transfer.
 *
 * Why this exists: TransferFactory_Transfer creates a
 * `TransferInstruction` contract on chain rather than a direct Holding.
 * The cETH doesn't actually land in the receiver's wallet until the
...
export async function getAcceptContext(
  ...
  if (!res.ok) {
    ...
    throw new LedgerCallError(
      res.status,
      `cETH accept-context registry rejected: ${res.status} ${text.slice(0, 240)}`
    );
  }
...
async function submitAcceptCommand(
  ...
  const commandId = `ceth-accept-${transferInstructionCid.slice(0, 16)}-${Date.now()}`;
  ...
  if (env.TRAFFIC_DEBUG === "1") {
    ...
    console.log(
      `[traffic] choice=cETH:TransferInstruction_Accept bodyBytes=${bodyBytes} disclosedBytes=${discBytes} disclosedCount=${disc.length}`
    );
  }
  ...
  //
  // Same retry shape as src/ceth/transfer.ts. Bounded: 6 attempts with
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    console.log(
      `[ceth.accept] sequencer 409 attempt ${attempt}/${MAX_ATTEMPTS}, retry in ${sleepMs}ms — cid=${commandId.slice(0, 16)}…`
    );
...
export async function acceptTransferInstruction(
  ...
  if (env.CETH_ENABLED !== "1" && env.CETH_ENABLED !== "true") {
    throw new LedgerCallError(503, "cETH is not enabled.");
  }
  ...
  // cETH ships with decimals=10. The registry's `decimals` field is the
  ...
  for (const ev of result.createdEvents) {
    ...
    if (arg.amount != null) {
      const ceth = Number(arg.amount);
      if (Number.isFinite(ceth)) {
        amountSat = Math.round(ceth * conversionFactor);
      }
    }
    break;
  }
