/**
 * cETH Token Standard registry client.
 *
 * Endpoint pattern:
 *
 *   GET {CETH_REGISTRY_URL}/api/token-standard/v0/registrars/{admin}/registry/metadata/v1/instruments
 *
 * Returns metadata for every instrument the admin party governs. For the
 * onrails cETH admin party that's just cETH today.
 *
 ...
 *      `[ceth.registry] DRIFT` lines in their tail when a DAR upgrade
 *      lands.
 ...
 *      take down our cETH paths if we have a recent value to keep
 *      going on.
 *
...
export class CethConfigError extends Error {}
...
function requireEnv(env: Env): {
  ...
  if (!env.CETH_REGISTRY_URL) {
    throw new CethConfigError(
      "CETH_REGISTRY_URL not set. Add the registry URL to env (MainNet: https://api.utilities.digitalasset.com)."
    );
  }
  if (!env.CETH_ADMIN_PARTY) {
    throw new CethConfigError(
      "CETH_ADMIN_PARTY not set. Add the onrails cETH admin party id (MainNet: rails-cethMain-1::1220…)."
    );
  }
  if (!env.CETH_INSTRUMENT_ID) {
    throw new CethConfigError(
      'CETH_INSTRUMENT_ID not set. For cETH use the literal string "cETH".'
    );
  }
  return {
    registryUrl: env.CETH_REGISTRY_URL.replace(/\/$/, ""),
    adminParty: env.CETH_ADMIN_PARTY,
    instrumentId: env.CETH_INSTRUMENT_ID,
  };
}

/**
 * Fetch and cache cETH instrument metadata from the registry.
 *
 ...
 * transfer failures.
 */
export async function getCethInstrument(env: Env): Promise<InstrumentMetadata> {
  const now = Date.now();
  ...
  if (!res.ok) {
    return serveStaleOrThrow(
      `cETH registry returned ${res.status} for ${url}`
    );
  }
  ...
  try {
    ...
    return serveStaleOrThrow(
      `cETH registry returned malformed JSON: ${err instanceof Error ? err.message : String(err)}`
    );
  }
  ...
  const meta: InstrumentMetadata = {
    ...
    name: hit.name,
    symbol: hit.symbol,
    // cETH ships with decimals=10. Use whatever the registry says; if
  ...
  if (cache) {
    ...
    if (before !== after) {
      console.warn(
        `[ceth.registry] DRIFT — instrument signature changed.\n  before: ${before}\n  after:  ${after}\n  (Most likely a DAR upgrade. Verify env CETH_ADMIN_PARTY is still correct.)`
      );
    }
    if (meta.instrumentId.admin !== adminParty) {
      console.warn(
        `[ceth.registry] ADMIN MISMATCH — env CETH_ADMIN_PARTY=${adminParty.slice(0, 32)}… but registry returned ${meta.instrumentId.admin.slice(0, 32)}…. Update the env var when convenient; transfers will use the resolved value.`
      );
    }
  }
  ...
  if (env.SPLICE_DEBUG === "1") {
    console.log(
      `[ceth.registry] cached metadata — symbol=${meta.symbol ?? "?"} decimals=${meta.decimals} package=${meta.packageId?.slice(0, 24)}…`
    );
  }
  return meta;
}
 ...
 *  has its OWN indexer that returns ONLY cETH contracts by definition —  *
...
/**
 * Fetch active cETH holdings owned by a specific party, via the
 * registry holdings endpoint. Bypasses the participant's 200-element
 * cap entirely — the registry returns ALL of the party's cETH holdings
 * regardless of count, since it's a token-scoped index.
 *
 * Optional `instrumentIdOverride` lets callers force a specific
 * instrument; default uses the env's CETH_INSTRUMENT_ID.
 *
...
export async function getActiveHoldings(
  ...
  try {
    ...
    throw new Error(
      `cETH registry holdings query network error (${url}): ${err instanceof Error ? err.message : String(err)}`
    );
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `cETH registry holdings query ${res.status} from ${url}: ${errBody.slice(0, 500)}`
    );
  }
...
function serveStaleOrThrow(reason: string): InstrumentMetadata {
  ...
  if (cache && now - cache.lastFreshAt < STALE_FALLBACK_MAX_MS) {
    console.warn(
      `[ceth.registry] STALE — serving cached metadata (lastFreshAt=${new Date(cache.lastFreshAt).toISOString()}). reason=${reason}`
    );
  ...
  throw new CethConfigError(reason);
}
