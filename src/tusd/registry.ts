/**
 * tUSD Token Standard registry client.
 *
 * Endpoint pattern:
 *
 *   GET {TUSD_REGISTRY_URL}/api/token-standard/v0/registrars/{admin}/registry/metadata/v1/instruments
 *
 * Returns metadata for every instrument the admin party governs. For the
 * onrails tUSD admin party that's just tUSD today.
 *
 ...
 *      `[tusd.registry] DRIFT` lines in their tail when a DAR upgrade
 *      lands.
 ...
 *      take down our tUSD paths if we have a recent value to keep
 *      going on.
 *
...
export class TusdConfigError extends Error {}
...
function requireEnv(env: Env): {
  ...
  if (!env.TUSD_REGISTRY_URL) {
    throw new TusdConfigError(
      "TUSD_REGISTRY_URL not set. Add the registry URL to env (MainNet: https://api.utilities.digitalasset.com)."
    );
  }
  if (!env.TUSD_ADMIN_PARTY) {
    throw new TusdConfigError(
      "TUSD_ADMIN_PARTY not set. Add the onrails tUSD admin party id (MainNet: rails-tusdMain-1::1220…)."
    );
  }
  if (!env.TUSD_INSTRUMENT_ID) {
    throw new TusdConfigError(
      'TUSD_INSTRUMENT_ID not set. For tUSD use the literal string "tf-usdt".'
    );
  }
  return {
    registryUrl: env.TUSD_REGISTRY_URL.replace(/\/$/, ""),
    adminParty: env.TUSD_ADMIN_PARTY,
    instrumentId: env.TUSD_INSTRUMENT_ID,
  };
}

/**
 * Fetch and cache tUSD instrument metadata from the registry.
 *
 ...
 * transfer failures.
 */
export async function getTusdInstrument(env: Env): Promise<InstrumentMetadata> {
  const now = Date.now();
  ...
  if (!res.ok) {
    return serveStaleOrThrow(
      `tUSD registry returned ${res.status} for ${url}`
    );
  }
  ...
  try {
    ...
    return serveStaleOrThrow(
      `tUSD registry returned malformed JSON: ${err instanceof Error ? err.message : String(err)}`
    );
  }
  ...
  const meta: InstrumentMetadata = {
    ...
    name: hit.name,
    symbol: hit.symbol,
    // tUSD ships with decimals=10. Use whatever the registry says; if
  ...
  if (cache) {
    ...
    if (before !== after) {
      console.warn(
        `[tusd.registry] DRIFT — instrument signature changed.\n  before: ${before}\n  after:  ${after}\n  (Most likely a DAR upgrade. Verify env TUSD_ADMIN_PARTY is still correct.)`
      );
    }
    if (meta.instrumentId.admin !== adminParty) {
      console.warn(
        `[tusd.registry] ADMIN MISMATCH — env TUSD_ADMIN_PARTY=${adminParty.slice(0, 32)}… but registry returned ${meta.instrumentId.admin.slice(0, 32)}…. Update the env var when convenient; transfers will use the resolved value.`
      );
    }
  }
  ...
  if (env.SPLICE_DEBUG === "1") {
    console.log(
      `[tusd.registry] cached metadata — symbol=${meta.symbol ?? "?"} decimals=${meta.decimals} package=${meta.packageId?.slice(0, 24)}…`
    );
  }
  return meta;
}
 ...
 *  has its OWN indexer that returns ONLY tUSD contracts by definition —  *
...
/**
 * Fetch active tUSD holdings owned by a specific party, via the
 * registry holdings endpoint. Bypasses the participant's 200-element
 * cap entirely — the registry returns ALL of the party's tUSD holdings
 * regardless of count, since it's a token-scoped index.
 *
 * Optional `instrumentIdOverride` lets callers force a specific
 * instrument; default uses the env's TUSD_INSTRUMENT_ID.
 *
...
export async function getActiveHoldings(
  ...
  try {
    ...
    throw new Error(
      `tUSD registry holdings query network error (${url}): ${err instanceof Error ? err.message : String(err)}`
    );
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `tUSD registry holdings query ${res.status} from ${url}: ${errBody.slice(0, 500)}`
    );
  }
...
function serveStaleOrThrow(reason: string): InstrumentMetadata {
  ...
  if (cache && now - cache.lastFreshAt < STALE_FALLBACK_MAX_MS) {
    console.warn(
      `[tusd.registry] STALE — serving cached metadata (lastFreshAt=${new Date(cache.lastFreshAt).toISOString()}). reason=${reason}`
    );
  ...
  throw new TusdConfigError(reason);
}
