/**
 * tUSD transfer — Splice Token Standard `TransferFactory_Transfer` exercise.
 *
 * Mirrors tusd's CBTC sibling. The pattern:
 *
 ...
 *
 * AUTH: Bearer token = onrails tUSD Keycloak access token (see
...
import type { TusdTransferResult, InstrumentId } from "./types";
 ...
 *  consume. We list the sender's tUSD Holdings, then pass ALL of them   *
 ...
 *  The REAL workaround for the operator is the tusd_holding_cache table *
 *  (see holding-cache.ts). Treasury hits the cache; user parties (which *
 *  have <200 active contracts each) work fine via wildcard.             *
 * ──────────────────────────────────────────────────────────────────── */

/**
 * Public, cache-bypassing variant of listSenderTusdHoldings. Used by the
 * merge-fat-holdings admin endpoint to enumerate ALL active tUSD Holdings
...
export async function listAllHoldingsForPartyFresh(
  ...
  return listSenderTusdHoldingsImpl(env, party, /* skipCache */ true);
}

async function listSenderTusdHoldings(
  env: Env,
  senderParty: string,
  ...
  void resolvedAdmin;
  return listSenderTusdHoldingsImpl(env, senderParty, /* skipCache */ false, minAmountSat);
}

async function listSenderTusdHoldingsImpl(
  env: Env,
  senderParty: string,
  skipCache: boolean,
  minAmountSat?: number
  ...
  // Plan B for the 200-element cap: if the tusd_holding_cache table
  ...
  // transferTusd with the new "change Holding" cid, so it stays current
  ...
  if (!skipCache) {
    try {
      ...
      if (cached && (minAmountSat === undefined || cached.amountRaw >= minAmountSat)) {
        console.log(
          `[tusd.transfer] listSenderTusdHoldings cache HIT sender=${senderParty.slice(0, 30)} cid=${cached.holdingCid.slice(0, 24)}… amount=${cached.amountRaw}`
        );
        return [cached.holdingCid];
      }
      console.log(
        `[tusd.transfer] listSenderTusdHoldings cache ${cached ? "INSUFFICIENT" : "MISS"} sender=${senderParty.slice(0, 30)} need=${minAmountSat ?? "?"} — walking ledger`
      );
    } catch (err) {
      // DB failures shouldn't break transfers — fall through to participant.
      console.warn(
        `[tusd.transfer] holding-cache read failed sender=${senderParty.slice(0, 30)}: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  }
  // ── End cache fast-path ──
  ...
  if (operatorParty && senderParty === operatorParty) {
    ...
    if (cids.length === 0) {
      throw new LedgerCallError(
        500,
        `Operator ${senderParty.slice(0, 32)}… has no spendable tUSD Holdings (ledger walk).`
      );
    }
    return cids;
  }
  ...
  if (!ledgerToken) {
    throw new LedgerCallError(
      500,
      "JSON_LEDGER_ADMIN_TOKEN not set — required to enumerate the sender's tUSD Holdings."
    );
  }

  // Resolve the tUSD Holding template + interface ids. Both are
  ...
  // tusd_holding_cache table (see holding-cache.ts). Treasury hits the
  ...
  const packageId =
    instrument.packageId ?? env.TUSD_HOLDING_TEMPLATE_PACKAGE_ID ?? null;
  const tusdHoldingTemplateId = packageId
    ? `${packageId}:Utility.Registry.Holding:Holding`
    : null;
  const tusdHoldingInterfaceId = env.TUSD_HOLDING_INTERFACE_PACKAGE_ID
    ? `${env.TUSD_HOLDING_INTERFACE_PACKAGE_ID}:Splice.Api.Token.HoldingV1:Holding`
    : null;
  ...
  // is skipped — they hit the tusd_holding_cache fast-path above.
  ...
  void tusdHoldingTemplateId;
  void tusdHoldingInterfaceId;
  ...
  const templateLabel = tusdHoldingTemplateId
    ? tusdHoldingTemplateId.slice(0, 40) + "…"
    : "<wildcard>";
  if (!res.ok) {
    const errBody = await res.text().catch(() => "<read failed>");
    console.error(
      `[tusd.transfer] listSenderTusdHoldings ${res.status} from ${ledgerHost} ` +
  ...
  console.log(
    `[tusd.transfer] listSenderTusdHoldings ok sender=${senderParty.slice(0, 30)} ` +
...
type TransferPayload = {
   ...
   *  registry (tUSD ships at decimals=10). */
  amount: string;
  ...
  /** Sender's tUSD holding cids the factory may consume.
...
async function getTransferFactory(
  ...
  // Use the RESOLVED admin from getTusdInstrument(), not the raw env
  ...
  if (!res.ok) {
    ...
    throw new LedgerCallError(
      res.status,
      `tUSD transfer-factory registry rejected: ${res.status} ${text.slice(0, 240)}`
    );
  }
...
async function submitTransferCommand(
  ...
  if (!ledgerHost) {
    throw new LedgerCallError(
      500,
      "JSON_API_URL not set. The tUSD transfer needs the same ledger host as CC."
    );
  }
  // Same reason as tusd/accept.ts: the participant rejects the Keycloak
  ...
  // every Token Standard asset — same for CBTC and tUSD.
    ...
    commands: [
      {
        ExerciseCommand: {
          ...
          choiceArgument: {
            ...
            // handling) — env.TUSD_ADMIN_PARTY may be stale if the
  ...
  if (env.TRAFFIC_DEBUG === "1") {
    ...
    console.log(
      `[traffic] choice=tUSD:TransferFactory_Transfer bodyBytes=${bodyBytes} disclosedBytes=${discBytes} disclosedCount=${disclosedContracts.length}`
    );
  }
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    console.log(
      `[tusd.transfer] sequencer 409 attempt ${attempt}/${MAX_ATTEMPTS}, retry in ${sleepMs}ms — cid=${commandId.slice(0, 12)}…`
    );
...
export async function transferTusd(
  env: Env,
  senderParty: string,
  ...
  if (env.TUSD_ENABLED !== "1" && env.TUSD_ENABLED !== "true") {
    throw new LedgerCallError(
      503,
      "tUSD is not enabled in this environment. Set TUSD_ENABLED=1 once credentials + participant config are in place."
    );
  }
  if (amountSat <= 0) {
    throw new LedgerCallError(400, "tUSD transfer amount must be positive.");
  }
  if (senderParty === recipientParty) {
    throw new LedgerCallError(400, "Sender and recipient are the same party.");
  }
  if (!env.TUSD_ADMIN_PARTY) {
    throw new LedgerCallError(500, "TUSD_ADMIN_PARTY not set.");
  }
  ...
  // env.TUSD_ADMIN_PARTY is just the bootstrap hint to find the right
  ...
  // claim. The decimals field is ALSO live-resolved — tUSD ships at
  ...
  // ── Pre-pick sender's tUSD holdings. The registry rejects empty
  //    arrays with "No holdings provided" — they want us to list the
  //    candidate UTXOs the factory may consume.
  const inputHoldingCids = await listSenderTusdHoldings(
    env,
    senderParty,
    resolvedAdmin,
    amountSat
  ...
  if (inputHoldingCids.length === 0) {
    throw new LedgerCallError(
      400,
      `Sender ${senderParty.slice(0, 32)}… has no spendable tUSD Holdings.`
    );
  }
  ...
  const transfer: TransferPayload = {
    ...
    // pass ALL tUSD Holdings owned by the sender; the factory picks
  ...
  try {
    ...
    // re-seed from a fresh inbound tUSD.
    ...
    if (
      ...
      try {
        ...
        console.warn(
          `[tusd.transfer] cache INVALIDATE (registry stale-cid) sender=${senderParty.slice(0, 30)} ` +
            `— registry rejected pre-flight: "${msg.slice(0, 120)}". Cache cleared.`
        );
      } catch (cacheErr) {
        console.error(
          `[tusd.transfer] failed to invalidate stale cache sender=${senderParty.slice(0, 30)}: ${cacheErr instanceof Error ? cacheErr.message : String(cacheErr)}`
        );
      }
    }
    throw err;
  ...
  try {
    ...
    // accept-cron (when an inbound tUSD lands) OR fails fast with
    ...
    if (isStaleCid) {
      try {
        ...
        await invalidateCachedHolding(db, senderParty);
        console.warn(
          `[tusd.transfer] cache INVALIDATE (stale cid recovery) sender=${senderParty.slice(0, 30)} ` +
  ...
  //   (b) Two-phase TransferInstruction — receiver's tUSD sits in a
  ...
  try {
    ...
    if (senderChangeCid) {
      ...
      console.log(
        `[tusd.transfer] cache UPDATE sender=${senderParty.slice(0, 30)} ` +
      ...
      await invalidateCachedHolding(db, senderParty);
      console.warn(
        `[tusd.transfer] cache INVALIDATE sender=${senderParty.slice(0, 30)} ` +
          `— no senderChangeCid in transaction tree; next call must re-seed`
      );
    }
  } catch (err) {
    console.error(
      `[tusd.transfer] cache update failed sender=${senderParty.slice(0, 30)}: ${err instanceof Error ? err.message : String(err)}`
    );
  }
